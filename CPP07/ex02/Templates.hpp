...
public:
  ...
  Array<T> &operator=(const Array<T> &idk) {
    if (this == &idk)
      return *this;
    delete[] _array;
    _len = idk._len;
  ...
  T &operator[](unsigned int id) {
    if (id >= _len)
      throw std::logic_error("Hasta la vista, error");
    return _array[id];
  }
  ...
  int find_first(int numb) const {
    for (int cnt = 0; cnt < _len; cnt++) {
      if (static_cast<int>(_array[cnt]) == numb)
        return cnt;
    }
    return (-1);
  }
  ...
  int find_biggest() const {
    ...
    if (_len > 0)
      ...
      temp = -1;
    for (int cnt = 0; cnt < _len; cnt++) {
      if (static_cast<int>(_array[cnt]) > static_cast<int>(_array[temp]))
        temp = cnt;
    }
    return (temp);
  }
